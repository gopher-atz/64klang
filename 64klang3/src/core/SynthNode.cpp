	...
	NODE_UPDATE_BEGIN   
		...
		*pattern++ = INP(TRIGGERSEQ_PATTERN12_15R).i[0];
		...
		if (s_testz_si128(n->v[0], n->v[0]))
		{
			...
			n->v[2].p[0] = (uintptr_t)(pdata+0);
			n->v[3].p[0] = (uintptr_t)(pdata+4);
		}
#endif		
...
#else
	if (((char*)(n->v[2].p[0]))[pindex.i[0]] & sindex.i[0])
		n->out.d[0] = SC[S_1_0].d[0];
	if (((char*)(n->v[3].p[0]))[pindex.i[1]] & sindex.i[1])
		n->out.d[1] = SC[S_1_0].d[0];
#endif
}
#endif
...
void SYNTHCALL SAMPLEREC_tick(SynthNode* n)
{
	...
	if (s_testz_si128(n->e, n->e))
	{
		...
		n->v[11].p[0] = (uintptr_t)(n->customMem);		
...
{	
	...
	sample_t scur = sample_t((((sample_t*)(n->v[11].p[0]))[curi.i[0]]).d[0], (((sample_t*)(n->v[11].p[0]))[curi.i[1]]).d[1]);
	sample_t snext = sample_t((((sample_t*)(n->v[11].p[0]))[nexti.i[0]]).d[0], (((sample_t*)(n->v[11].p[0]))[nexti.i[1]]).d[1]);
	return s_lerp(scur, snext, i-cur);
}

#ifndef SAMPLER_SKIP
#ifdef CODE_SECTIONS
K64_CODE_SECTION(".sn3b")
#endif
	...
	NODE_UPDATE_BEGIN   
		...
		if ((mode & SAMPLER_MODEMASK) == SAMPLER_STORED)
		{
...
#endif
			{
				...
				// buffer start pointer
				n->v[11].p[0] = (uintptr_t)(&(SynthGlobalState.RawWaveTable[table][1]));
			}
		}
...
#else // !_WIN32
// SAPI is Windows-only; on other platforms SAPI_tick is a no-op
#ifdef CODE_SECTIONS
K64_CODE_SECTION(".sn3f")
#endif
void SYNTHCALL SAPI_tick(SynthNode* n)
{
	NODE_STATEFUL_PROLOG;
	// SAPI TTS not available on this platform
}
#endif // _WIN32
#endif // SAPI_SKIP
...
void SYNTHCALL GMDLS_tick(SynthNode* n)
{
	...
	n->v[11].p[0] = (uintptr_t)(GMDLS_SampleBuffer[mode]);
}
#endif
...
void SYNTHCALL BOWED_tick(SynthNode* n)
{
	...
	NODE_UPDATE_BEGIN   
		...
		// delay pointers
		n->v[12].p[0] = (uintptr_t)(n->customMem);
		n->v[13].p[0] = (uintptr_t)(n->customMem + 9840 * 4);
	...
	sample_t bpos = s_toInt(n->v[0]);
	(((sample_t*)n->v[12].p[0])[bpos.i[0]]).d[0] = neckReflection.d[0];
	(((sample_t*)n->v[12].p[0])[bpos.i[1]]).d[1] = neckReflection.d[1];

	// neck delay new input	
	bpos = s_toInt(n->v[1]);
	(((sample_t*)n->v[13].p[0])[bpos.i[0]]).d[0] = bridgeReflection.d[0];
	(((sample_t*)n->v[13].p[0])[bpos.i[1]]).d[1] = bridgeReflection.d[1];
