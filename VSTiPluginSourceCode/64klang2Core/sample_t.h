...
_MM_ALIGN16 union complexsample_t
{
	double  d[4];
	sample_t pd[2];
	struct
	...
	// construction
	inline complexsample_t() {}
	inline complexsample_t(const complexsample_t& x) { *this = x; }
	inline complexsample_t(const sample_t& x) { re = x.pd; im = sample_t::zero(); }
	inline explicit complexsample_t(const sample_t& x1, const sample_t& x2) { re = x1; im = x2; }
	inline explicit complexsample_t(const double x) { *this = complexsample_t(sample_t(x)); }
	inline explicit complexsample_t(double x1, double x2) { *this = complexsample_t(sample_t(x1), sample_t(x2)); }

	// assignment
	inline complexsample_t& operator=(const complexsample_t& x) { re = x.re; im = x.im; return *this; }

	// arithmetic	 
	inline complexsample_t& operator+=(const complexsample_t& x);
	inline complexsample_t& operator-=(const complexsample_t& x);
	inline complexsample_t& operator*=(const complexsample_t& x);
};

// operator overloads
inline complexsample_t		operator+(const complexsample_t& a, const complexsample_t& b) { return complexsample_t(a.re+b.re, a.im+b.im); }
inline complexsample_t		operator-(const complexsample_t& a, const complexsample_t& b) { return complexsample_t(a.re-b.re, a.im-b.im); }
inline complexsample_t		operator*(const complexsample_t& a, const complexsample_t& b) { return complexsample_t(a.re*b.re - a.im*b.im, a.re*b.im + a.im*b.re); }

// arithmetic
inline complexsample_t&		complexsample_t::operator+=(const complexsample_t& x) { return *this = *this + x; }
inline complexsample_t&		complexsample_t::operator-=(const complexsample_t& x) { return *this = *this - x; }
inline complexsample_t&		complexsample_t::operator*=(const complexsample_t& x) { return *this = *this * x; }

// other math functions
inline complexsample_t		c_exp(const complexsample_t& x) { sample_t er = s_exp(x.re); return complexsample_t(er*s_cos(x.im), er*s_sin(x.im)); }

extern void	__fastcall		c_fft(complexsample_t* x, int N);
extern void	__fastcall		c_ifft(complexsample_t* x, int N);
#endif
